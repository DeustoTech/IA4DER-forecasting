...
for(archivo in archivos) {
  ...
  calcular_features <- function(serie, ID1, firstPanel) {
    ...
    colnames(injection_by_hour_weekdays) <- c(paste("mean_VAL_AE_weekdays_", 0:23))
    ...
    colnames(injection_by_hour_weekend) <- c(paste("mean_VAL_AE_weekend_", 0:23))
    ...
    colnames(consumption_by_week) <- c(paste0("total_VAL_AI_week_", 1:max(consumption_by_week$week, na.rm = T)))    

    consumption_by_month <- serie %>%
      group_by(month) %>%
      summarise(total_VAL_AI = sum(VAL_AI, na.rm = TRUE)) %>% pivot_wider(names_from = week, values_from = total_VAL_AI)
    colnames(consumption_by_month) <- c(paste0("total_VAL_AI_month_", 1:12))    
    ...
    colnames(inyection_by_week) <- c(paste0("total_VAL_AI_week_", 1:max(inyection_by_week$week, na.rm = T)))    
    
    inyection_by_month <- serie %>%
      group_by(month) %>%
      summarise(total_VAL_AE = sum(VAL_AE, na.rm = TRUE)) %>% pivot_wider(names_from = week, values_from = total_VAL_AE)
    colnames(inyection_by_month) <- c(paste0("total_VAL_AI_month_", 1:12)) 
    ...
    feats <- data.frame(
      ...
      firstPanel_timestamp =  firstPanel
  ...
  features_sin_auto <- if (any(serie$AUTO == 0)) calcular_features(serie %>% filter(AUTO == 0), ID1 = id_serie, firstPanel = firstPanel)
  features_con_auto <- if (any(serie$AUTO == 1)) calcular_features(serie %>% filter(AUTO == 1), ID1 = id_serie, firstPanel = firstPanel)
