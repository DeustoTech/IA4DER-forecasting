...
regresion_model_feats <- function(model_type, modeloE, target_variable, trainSet, testSet, testID) {
  ...
  trainSet[[target_variable]] <- ifelse(is.na(trainSet[[target_variable]]), mean(trainSet[[target_variable]], na.rm = TRUE), trainSet[[target_variable]])
  trainSet <- trainSet[trainSet[[target_variable]] >= -1, ] # Eliminar valores problemáticos
 
  trainSet[[log_variable]] <- log(trainSet[[target_variable]] + 1) 
  testSet[[log_variable]] <- log(testSet[[target_variable]] + 1)
  
  trainSet <- trainSet[is.finite(trainSet[[log_variable]]), ] # Eliminar Inf y NaN
